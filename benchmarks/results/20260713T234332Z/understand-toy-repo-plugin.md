...
1. Module loading initializes `attempts = []` ([paymentAttempts.js:1](../../../examples/toy-buggy-repo/src/paymentAttempts.js#L1)).
2. `createPaymentAttempt` creates an ID from the current array length and copies `invoiceId`, `idempotencyKey`, and `amountCents` into a new object ([paymentAttempts.js:3](../../../examples/toy-buggy-repo/src/paymentAttempts.js#L3)).
3. It assigns the documented `type: "FooPaymentAttempt"` discriminator ([paymentAttempts.js:9](../../../examples/toy-buggy-repo/src/paymentAttempts.js#L9)).
4. It unconditionally appends and returns that object ([paymentAttempts.js:12](../../../examples/toy-buggy-repo/src/paymentAttempts.js#L12)).
5. `allPaymentAttempts()` returns a shallow array copy ([paymentAttempts.js:16](../../../examples/toy-buggy-repo/src/paymentAttempts.js#L16)). Changing that returned array does not change the store, but changing one of its record objects does.

## Documentation versus implementation

| Claim | Verdict |
|---|---|
| Attempts are idempotent and duplicate retries reuse the original ([README.md:7](../../../examples/toy-buggy-repo/README.md#L7), [README.md:8](../../../examples/toy-buggy-repo/README.md#L8)) | **Contradicted.** There is no lookup, comparison, or reuse branch. |
| A retry with the same `idempotencyKey` reuses the existing record ([api.md:5](../../../examples/toy-buggy-repo/docs/api.md#L5)) | **Contradicted.** A duplicate-key probe produced `count: 2`, IDs `attempt_1` and `attempt_2`, and `sameObject: false`. |
| Records use `FooPaymentAttempt` ([api.md:3](../../../examples/toy-buggy-repo/docs/api.md#L3)) | **Matches at the in-memory object level.** Both probed records had that type. |
| “Serialized” records use that type ([README.md:9](../../../examples/toy-buggy-repo/README.md#L9)) | **Not demonstrated.** The fixture contains no serialization or persistence layer. |
...
- If manually invoked in a fresh process, the function creates one record and checks only that the array length is one ([paymentAttempts.test.js:4](../../../examples/toy-buggy-repo/tests/paymentAttempts.test.js#L4), [paymentAttempts.test.js:10](../../../examples/toy-buggy-repo/tests/paymentAttempts.test.js#L10)).
